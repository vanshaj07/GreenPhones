...
class DeviceQualityCheckQuestionsScreenSecond extends Component {
    constructor(props) {
        super(props);
        this.state = {
        //     phone_problem_1: {
...
showProblems=()=>{
    const {phone_problem_1, problemsData} = this.props;
    const {answers} = phone_problem_1;
    let problem = null
    if(problemsData.length>0){
        problem =  problemsData[0].options.map((item, index) =>{
            return(
<div class=" my-2 mx-2 col-sm-6 col-md-3" key ={index}  style={{width: "11.8rem"}}>
<label class="label-class">
    <input type="checkbox" checked={answers.includes(item.id)} onClick={()=>{this.props.handleProblem1(item.id, item.price)}} />
...
export default DeviceQualityCheckQuestionsScreenSecond;